def setup_chat_views
  ...
  create_file "app/views/layouts/chat.html.erb", <<~ERB
    ...
    <html>
      <head>
        <title><%= content_for(:title) || "AI Chat" %></title>
  ...
  create_file "app/views/conversations/index.html.erb", <<~ERB
    <div class="w-64 bg-slate-950 border-r border-slate-800 flex flex-col">
      ...
      <div class="p-4">
        <%= button_to "+ New Chat", conversations_path, class: "w-full bg-blue-600 hover:bg-blue-500 text-white py-2 px-4 rounded transition text-sm font-medium cursor-pointer" %>
      </div>
  ...
  # 3. Conversation Show (The Chat Interface)
  create_file "app/views/conversations/show.html.erb", <<~ERB
    <div class="flex h-full w-full" data-controller="loader">
      <!-- Sidebar (Duplicate for now, ideally a partial) -->
      <div class="w-64 bg-slate-950 border-r border-slate-800 flex flex-col shrink-0 hidden md:flex">
        <div class="p-4 border-b border-slate-800">
          <h1 class="font-bold text-xl tracking-tight text-white"><%= link_to "Recon AI", conversations_path %></h1>
        </div>
        <div class="p-4">
          <%= button_to "+ New Chat", conversations_path, class: "w-full bg-blue-600 hover:bg-blue-500 text-white py-2 px-4 rounded transition text-sm font-medium cursor-pointer" %>
        </div>
        <div class="flex-1 overflow-y-auto px-2 space-y-1">
          <% Conversation.order(updated_at: :desc).limit(20).each do |conv| %>
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-slate-900">
        ...
        <div data-loader-target="indicator" class="hidden px-6 pb-4 bg-slate-900 flex flex-col items-start animate-pulse border-t border-slate-800 pt-4">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-slate-500 font-mono ml-2">Agent is thinking...</span>
          </div>
        </div>
        ...
        <div class="p-4 bg-slate-900">
          <%= form_with url: conversation_messages_path(@conversation), 
            ...
            <%= f.text_area :content, 
                ...
                class: "w-full bg-slate-800 border-slate-700 rounded-xl px-4 py-3 pr-12 text-white focus:ring-2 focus:ring-blue-600 focus:border-transparent resize-none",
                rows: 1,
